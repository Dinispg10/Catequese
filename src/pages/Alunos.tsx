...
export default function Alunos() {
  ...
  const loadReference = async () => {
    const [{ data: paroquiasData }, { data: escolasData }, { data: catequistasData }, { data: centrosData }] = await Promise.all([
  ...
  const loadAlunos = async () => {
    ...
    if (search.trim()) {
      query = query.ilike('nome_aluno', `%${search.trim()}%`);
    }
  ...
  const paroquiaOptions = useMemo(() => paroquias.map((item) => ({ value: item.id, label: item.nome })), [paroquias]);
  const escolaOptions = useMemo(() => escolas.map((item) => ({ value: item.id, label: item.nome })), [escolas]);
        ...
        <div className="card">
          ...
          <div className="form">
            ...
            <SelectField
              ...
              placeholder="Selecionar"
            />
            <FormField label="Nome do aluno" value={selected.nome_aluno} onChange={(value) => setSelected({ ...selected, nome_aluno: value })} required />
            <div className="grid two">
              <FormField label="Nome do pai" value={selected.nome_pai ?? ''} onChange={(value) => setSelected({ ...selected, nome_pai: value })} />
              <FormField label="Nome da mãe" value={selected.nome_mae ?? ''} onChange={(value) => setSelected({ ...selected, nome_mae: value })} />
            </div>
            <div className="grid two">
              <FormField
            ...
            <CheckboxField label="Batizado" checked={selected.batizado} onChange={(value) => setSelected({ ...selected, batizado: value })} />
            {selected.batizado && (
              <div className="grid two">
                <FormField
                  label="Data batismo"
            ...
            />
            <FormField label="Morada" value={selected.morada ?? ''} onChange={(value) => setSelected({ ...selected, morada: value })} />
            <div className="grid two">
              <FormField
                label="Localidade"
            ...
            <div className="grid two">
              ...
              />
              <FormField label="Email" value={selected.email ?? ''} onChange={(value) => setSelected({ ...selected, email: value })} />
            </div>
            <div className="grid two">
              <FormField
                label="Ano catecismo"
            ...
            <div className="form-actions">
              <button className="button" type="button" onClick={() => setSelected(emptyAluno)}>
                Novo
              </button>
